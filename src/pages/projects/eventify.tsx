...
const Eventify: React.FC = () => {
  return (
    <div className="min-h-screen p-4 bg-custom-grey flex flex-col items-center justify-start">
      <Navbar textColor='text-white' />
      <main className="container mx-auto mt-15" style={{ maxWidth: "1500px"}}>
        <ProjectCard 
          title="Eventify" 
...
const ProjectCard = ({ title, subtitle, details, imageUrl, projectUrl }: { title: string, subtitle: string, details: string, imageUrl: string, projectUrl: string }) => {
  return (
    <Link href={projectUrl}>
      <div className="project-card text-white flex flex-col md:flex-row items-center justify-between w-full mb-8">
        {/* Immagine a sinistra - occupa tutta la larghezza dello schermo solo su desktop */}
        <div className="w-screen md:relative md:left-1/2 md:right-1/2 md:-ml-[50vw] md:-mr-[50vw] flex-shrink-0">
          <Image
            src={imageUrl}
            alt={title}
            className="w-full h-full object-cover max-w-[800px] transition-transform duration-500"
            width={500}
            height={500}
          />
        </div>
        ...
        <div className="md:w-1/2 w-full lg:w-1/3 text-right md:text-right text-center p-4">
